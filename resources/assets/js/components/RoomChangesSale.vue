...
export default {
    ...
    methods: {
        ...
        fetchData: function() {
            this.$http.get('/projectCS314/public/api/products').then(function(products) {
        ...
        searchProduct: function(direct) {
            ...
            if (this.searchProductQuery.limit <= qLength || qLength == 0)
            {
                // set delay
                setTimeout(function() {
                    this.$http.get('/projectCS314/public/api/products', {
                        params : {
        ...
        createSale: function() {
            if (confirm('Press ok button to proceed'))
            {
                ...
                if(this.cart==null){
                    this.$http.post('/projectCS314/public/api/sales', {
                    ...
                    items: _.map(this.cart, function(cart){
                        ...
                        $.notify('Sales created with <a href="/projectCS314/public/sales/receipt/' + responseBody.id + '" target="_BLANK">INVOICE</a>', {
                            type: 'success',
                            placement: {
                                from: 'bottom'
                            }
                        })
                        window.open('/projectCS314/public/sales/receipt/' + responseBody.id)
                        window.location.href = '/projectCS314/public/';
                    ...
                    additionalTimeFee: this.price[0].additionalTimeFee,
                    corkageFee: this.price[0].corkageFee,
                    todayOrTommorrow: this.price[0].todayOrTommorrow,
